...
[[network-security-sase]] and [[dlp]] controls — those see packets and files; an
...
a hedge-fund CTO: if you are only running a governed [[enterprise-ai-assistant]]
...
becomes Day-1. Distinct from [[ai-access-governance]], which governs *employees
...
- [[prisma-airs]] — Palo Alto's AI runtime security (incl. former Protect AI capabilities); the incumbent-platform play, tied to the broader Prisma/Cortex stack.
- [[hiddenlayer]] — model-scanning heritage; runtime detection plus red-teaming (cross-listed in [[ai-red-teaming]]).
- [[witnessai]] — inline inspection of prompts/responses; bridges into [[ai-access-governance]] (its primary slot).
- [[pillar-security]] — full-lifecycle AI security, runtime guardrails plus posture.
- [[cisco-ai-defense]] — Cisco's runtime AI defense, built on the former Robust Intelligence tech.
- [[robust-intelligence]] — the acquired company now inside Cisco AI Defense (may merge into that page).
- [[aim-security]] — enterprise AI security/guardrails platform.
- [[enkrypt-ai]] — guardrails plus red-teaming (cross-listed in [[ai-red-teaming]]).
- [[calypsoai]] — runtime inference security and control.
- [[lakera]] — prompt-injection/guardrail detection with a strong research profile (cross-listed in [[ai-red-teaming]]).
- [[trojai]] — model and runtime security, scanning plus runtime defense.
- [[splxai]] — red-teaming-first vendor that also offers runtime guardrails (primary in [[ai-red-teaming]]).

**Runtime guardrails inside a DLP/data-security lineage**
- [[prompt-security]] — prompt/response inspection and AI DLP (cross-listed in [[dlp]] and [[ai-access-governance]]).
- [[lasso-security]] — runtime LLM/agent protection (cross-listed in [[dlp]] and [[agent-runtime-security]]).
...
incorporate the former Protect AI; CalypsoAI flagged as acquired by [[f5]]; Lakera
flagged as acquired by Check Point; Aim Security flagged as acquired by
[[cato-networks]]; Prompt Security flagged as acquired by [[sentinelone]]; Cisco AI
...
- [[ai-gateway]] — the gateway is the traffic-routing choke point; the firewall is the inspection logic. They are frequently sold or bundled together (gateway + inline guardrails).
- [[ai-access-governance]] — governs employees' use of external AI; runtime security protects internally built AI. Several vendors (WitnessAI, Prompt Security, Lasso) straddle both.
- [[ai-red-teaming]] — offline/pre-prod testing that finds the weaknesses the runtime firewall must block; many vendors do both (HiddenLayer, Lakera, SplxAI, Enkrypt).
- [[network-security-sase]] / [[dlp]] — the foundation-layer controls this layer sits on top of.
- [[agent-runtime-security]] — extends runtime protection from single-model calls to multi-step agent behavior.
...
- Heavy overlap with [[ai-red-teaming]] (HiddenLayer, Lakera, SplxAI, Enkrypt) and [[ai-access-governance]] (WitnessAI, Prompt Security, Lasso) — respondents may not distinguish "firewall" from "red-team" from "shadow-AI." Consider a clarifying lead-in defining runtime/inline.
...
- Where does the line sit between AI runtime security (protecting the app) and [[agent-runtime-security]] (protecting multi-step agents)? As agents proliferate the two may merge.
- Should `robust-intelligence` collapse into [[cisco-ai-defense]] once the acquisition and rebrand are confirmed?
