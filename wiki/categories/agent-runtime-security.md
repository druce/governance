...
just the user's prompt (that's [ai-runtime-security](ai-runtime-security.md)) but the agent's *plans, tool
...
[hitl-approvals](hitl-approvals.md) (human sign-off on high-stakes actions) and [trust-zone-segmentation](trust-zone-segmentation.md)
...
- [zenity](../vendors/zenity.md) — security and governance for agents and low-code/copilot platforms; runtime detection of risky agent behaviour. Cross-listed to [ai-spm](ai-spm.md).
- [lasso-security](../vendors/lasso-security.md) — AI/agent security plus shadow-AI and DLP; common shortlist for agent + LLM-app protection. Cross-listed to [ai-runtime-security](ai-runtime-security.md) and [dlp](dlp.md).
- [operant-ai](../vendors/operant-ai.md) — runtime protection for agentic apps and APIs (in-cluster/runtime defense angle).
- [apex-security](../vendors/apex-security.md) — enterprise agent security (per seed, acq. by Tenable — unverified).
- [straiker](../vendors/straiker.md) — runtime security for AI agents and applications.
- [noma-security](../vendors/noma-security.md) — AI/ML and agent security spanning discovery and runtime; primary in [ai-spm](ai-spm.md), cross-listed here.
...
agent security. Expect the same platform gravity as [ai-spm](ai-spm.md): cloud-security, EDR, and
...
- [ai-spm](ai-spm.md) — the discovery/posture sibling; AI-SPM inventories agents, this layer guards them at runtime. Largest overlap in the model/prompt layer (Zenity, Noma span both).
- [ai-runtime-security](ai-runtime-security.md) — the AI firewall for prompts/responses; agent-runtime extends the same idea to tool calls and multi-step agent loops.
- [mcp-gateway](mcp-gateway.md) — controls *which tools* an agent can reach; agent-runtime watches *how the agent behaves* once it has them. Complementary chokepoints.
- [authorization-engine](authorization-engine.md) — decides if a given agent action is allowed; agent-runtime can call into authz or enforce policy itself.
- [hitl-approvals](hitl-approvals.md) / [trust-zone-segmentation](trust-zone-segmentation.md) — process controls this layer backstops.
...
- **Heavy overlap with [ai-spm](ai-spm.md):** Zenity and Noma appear on both lists. If you ask both questions, a short scope line ("runtime *protection* of agents, not just inventory") reduces double-counting.
- Lasso also appears under [ai-runtime-security](ai-runtime-security.md) and shadow-AI; respondents may file it wherever they first met it.
- "Other" likely captures Prisma AIRS and AI-firewall vendors that pitch agent protection as a feature.

## Open taxonomy questions

- **This is the other half of the ai-spm overlap, and the messiest seam in the layer.** The seed *doc* has no separate row — agent security is folded into "AI-SPM / Agent Governance"; only the *CSV* breaks "Agent Security" out as its own survey question. We keep it separate (see Q1 in taxonomy-gaps.md) to preserve the survey cut and because runtime enforcement is genuinely a different job from posture/inventory — but the vendor lists bleed together (Zenity, Noma, Lasso). Working rule: **agent-runtime-security = enforcement at the agent's action boundary; [ai-spm](ai-spm.md) = visibility/posture over the AI estate.**
...
- Overlap with [mcp-gateway](mcp-gateway.md) and [authorization-engine](authorization-engine.md) (the "stop the bad action" job is shared across three layers) — see those pages' open questions.
