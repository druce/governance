...
**Where it sits in the stack** — [[siem-soc]], foundation layer. This is detection-and-response plumbing, not an AI-specific control, so its lethal-trifecta role is **none** directly — but it is increasingly where AI-related security signals land (Copilot/agent activity connectors, agent identity tables) and where an [[ai-soc-analysts]] layer would draw context. Lives across all trust zones as an observer.

**Deployment & architecture** — SaaS, runs on Azure; billed on data ingestion/retention. A 2025 redesign added the **Sentinel data lake** (GA Sep 2025) for cheaper long-term retention, a graph layer, and an **MCP server** (public preview) so AI agents (Security Copilot, or VS Code + GitHub Copilot) can query and act on security data over an open standard. Integrates with Microsoft Defender XDR ([[microsoft-defender]]), Entra, and third-party sources.

**Positioning & differentiators** — Tightly coupled to the Microsoft security ecosystem; strongest when Defender/Entra/M365 are the telemetry sources. The 2025-26 pivot is "agentic SOC": natural-language playbook generation (Python), no-code agent building in the Security Copilot portal, and agentic triage. Competes with [[google-secops]], [[splunk]], [[elastic]], [[crowdstrike-logscale]], [[sumo-logic]].
...
**Competitors / alternatives** — [[google-secops]], [[splunk]], [[elastic]], [[crowdstrike-logscale]], [[sumo-logic]].
