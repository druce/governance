...
Primary slot is [[ai-access-governance]] (CASB-for-AI / shadow-AI control — the access-governance and visibility job), with a strong secondary in [[ai-runtime-security]] (the inline Protect guardrails). Because it's an inline chokepoint on AI traffic, it touches all three lethal-trifecta legs: **untrusted-input** (prompt-injection/jailbreak blocking), **sensitive-data** (redaction/DLP on prompts), and **egress** (governing which AI destinations data can flow to). It's a yellow-zone enforcement point in front of model and agent calls.
