...
**Where it sits in the stack** — The [[llm-observability]] layer; its gateway features also overlap [[ai-gateway]]. As a proxy it sits on the model-traffic path, but it is an observability/routing layer, not a security firewall — it does not by itself break a leg of the lethal trifecta. Adjacent to [[litellm]], [[portkey]], [[openrouter]] on the gateway side.
