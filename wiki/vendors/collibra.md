...
Data layer for cataloging/governance, and [[ai-governance-platform]] for the AI-model-governance use. Lethal-trifecta role is indirect — it documents and classifies the **sensitive-data** leg (knowing what data is sensitive and where) but does not enforce access or block egress. The [[dspm]] tag is loose: Collibra catalogs and governs data, it is not a security-posture scanner like [[cyera]]/[[bedrock-security]]; treat its DSPM membership as adjacency, not equivalence.
