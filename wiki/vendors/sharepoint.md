...
**Where it sits in the stack** — [[content-sources]] in the retrieval layer (Day-1 *if* doing RAG). Lethal-trifecta role: it is the **sensitive-data** leg — the trove an attacker wants exfiltrated. Pairs with [[microsoft-purview]] (classification/DLP) and [[microsoft-graph]] (entitlement-aware retrieval).
