...
**Where it sits in the stack** — [[policy-as-code]], governance layer. Tightly coupled to [[terraform-cloud]] (its most common host) and [[hashicorp-vault]]. Lethal-trifecta role: indirect/meta — it doesn't sit in the prompt or data path; it governs the infrastructure and secrets workflows that everything else depends on, and underpins [[trust-zone-segmentation]] and [[promotion-gates]].
...
**Positioning & differentiators** — Proprietary and **HashiCorp-ecosystem-specific** — its strength (deep, native integration with Terraform/Vault) is also its limit. Contrast the open, general-purpose engines: [[open-policy-agent]] / [[styra]] (Rego, cloud-native, works across many systems) and [[kyverno]] (Kubernetes-native, YAML). Pick Sentinel when you are standardized on HashiCorp Enterprise; pick OPA/Kyverno for portability.
...
**CTO / hedge-fund lens** — Day-2. Relevant only if you run HashiCorp Enterprise/HCP and want policy gates on infrastructure and secrets. Not an AI-specific control, but a credible mechanism to enforce trust-zone and promotion-gate rules in IaC. If you are not a HashiCorp shop, [[open-policy-agent]] or [[kyverno]] is the more neutral choice.

**Competitors / alternatives** — [[open-policy-agent]], [[styra]], [[kyverno]].
