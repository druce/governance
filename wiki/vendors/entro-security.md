...
**Categories** — [[non-human-identity]] (primary), [[secrets-management]]
...
Foundation layer. Primary home is [[non-human-identity]] (the system of record and lifecycle for machine identities) with strong overlap into [[secrets-management]]. Unlike a vault (which stores secrets), Entro focuses on *discovery, posture, and lifecycle governance* across the secrets that already exist in many vaults and platforms. Lethal-trifecta role: constrains the **egress / action** leg (limiting what credentials can reach) and protects **sensitive-data** by finding and rotating exposed or over-scoped secrets.
...
Entro is an NHI-and-secrets *governance* layer rather than a vault — it sits over existing secret stores (cloud secret managers, [[hashicorp-vault]], [[cyberark]]/Conjur) to give one inventory, risk posture, and automated rotation. Its nearest neighbors are [[clutch-security]], [[token-security]], [[oasis-security]], [[aembit]], and [[astrix-security]]; vs. those it emphasizes the secrets-lifecycle (vaulting + rotation + decommissioning) angle. Differentiated from MCP-native agent brokers like [[natoma]] and [[mintmcp]], which govern the runtime tool call rather than the credential lifecycle.
...
[[clutch-security]], [[token-security]], [[oasis-security]], [[aembit]], [[astrix-security]], [[silverfort]]; vaults: [[hashicorp-vault]], [[cyberark]], [[azure-key-vault]], [[aws-secrets-manager]].
